...
- **steps** `array` - An array of notes to play
...
- **notes** `array` - An array of notes to trigger Instrument, useful for auditioning sounds or live performance.
...
- **oscillatorType** `triangle | sine | square` - A repeating waveform shape. Only for some synth types.
...
- **type** `feedbackDelay | distortion | freeverb` - Effect type
