...
- Update Instrument `notes` to not retrigger on every render. Revert back to `isPlaying` check and add `key` to `NoteType` that should be unique to trigger sound.
